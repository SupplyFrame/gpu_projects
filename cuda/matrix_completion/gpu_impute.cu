...
matrix_t * read_file(const char * inputfile,int rows,int cols){
    matrix_t * matrix = new matrix_t;
    matrix->n = rows;
    matrix->m = cols;
    matrix->data = new float[rows*cols];
    matrix->is_missing = new bool[rows*cols];
    ...
    for(int i=0;i<rows;++i){
        for(int j=0;j<cols;++j){
            fscanf(file_ptr,"%f",matrix->data+(j*rows+i));
            matrix->is_missing[j*rows+i] = matrix->data[j*rows+i]<0?true:false;
        }
    }
...
int main(int argc,char * argv[]){
    ...
    int n = atoi(argv[++arg]);
    int m = atoi(argv[++arg]);
    matrix_t * matrix = read_file(inputfile,n,m);
    int rank = 1;
    impute(matrix,rank);
    cleanup();
    delete matrix;
